...
extern void LCD_WriteBitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                            uint16_t* bitmap);
...
uint GFX_getWidth() { return _width; }

uint GFX_getHeight() { return _height; }

void GFX_setClearColor(uint16_t color) { clearColour = color; }

void GFX_clearScreen() { GFX_fillRect(0, 0, _width, _height, clearColour); }
      ...
      for (int8_t j = 0; j < 8; j++, line >>= 1) {
        if (line & 1) {
          if (size_x == 1 && size_y == 1)
            ...
            GFX_fillRect(x + i * size_x, y + j * size_y, size_x, size_y, color);
        } else if (bg != color) {
          if (size_x == 1 && size_y == 1)
            GFX_drawPixel(x + i, y + j, bg);
          else
            GFX_fillRect(x + i * size_x, y + j * size_y, size_x, size_y, bg);
        }
      }
    }
      ...
      for (xx = 0; xx < w; xx++) {
        ...
        if (bits & 0x80) {
          if (size_x == 1 && size_y == 1) {
            ...
            GFX_fillRect(x + (xo16 + xx) * size_x, y + (yo16 + yy) * size_y,
                         size_x, size_y, color);
          }
        }
        bits <<= 1;
...
void GFX_write(uint8_t c) {
  if (!gfxFont) {
    if (c == '\n') {               // Newline?
      ...
      GFX_drawChar(cursor_x, cursor_y, c, textcolor, textbgcolor, textsize_x,
                   textsize_y);
...
void GFX_setTextColor(uint16_t color) { textcolor = color; }

void GFX_setTextBack(uint16_t color) { textbgcolor = color; }
...
void printString(char s[]) {
  uint8_t n = strlen(s);
  for (int i = 0; i < n; i++) GFX_write(s[i]);
}
